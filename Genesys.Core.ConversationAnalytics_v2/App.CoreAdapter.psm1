...
function Initialize-CoreAdapter {
    ...
    Assert-Catalog -CatalogPath $CatalogPath -ErrorAction Stop
...
function Start-FullRun {
    ...
    $invokeParams = @{
        DatasetKey        = 'analytics-conversation-details'
        CatalogPath       = $script:CatalogPath
        OutputRoot        = $script:OutputRoot
        DatasetParameters = $DatasetParameters
    }
...
function Get-RecentRunFolders {
    ...
    $dirs = [System.IO.Directory]::GetDirectories($OutputRoot) |
            Where-Object { [System.IO.File]::Exists([System.IO.Path]::Combine($_, 'manifest.json')) } |
