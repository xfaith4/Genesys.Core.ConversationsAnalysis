...
function FileExists {
    ...
    return [System.IO.File]::Exists([System.IO.Path]::Combine($AppRoot, $RelPath))
}

function ReadFile {
    param([string]$RelPath)
    $full = [System.IO.Path]::Combine($AppRoot, $RelPath)
...
function AllAppFiles {
    ...
    $files = @('App.ps1','App.UI.ps1','App.CoreAdapter.psm1','App.Auth.psm1',
               'App.Config.psm1','App.Index.psm1','App.Export.psm1')
...
Check 'STR-08' 'XAML\MainWindow.xaml exists'       { FileExists 'XAML\MainWindow.xaml' }
Check 'STR-09' 'tests\Test-Compliance.ps1 exists'  { FileExists 'tests\Test-Compliance.ps1' }
Check 'STR-10' 'tests\Invoke-AllTests.ps1 exists'  { FileExists 'tests\Invoke-AllTests.ps1' }

# ── REST CALL ISOLATION (REST) ────────────────────────────────────────────────

Write-Host "`n=== REST ISOLATION (Gate D) ===" -ForegroundColor Cyan

$nonAuthContent = FilesExcluding -Exclude @('App.Auth.psm1')
...
Check 'INIT-05' 'App.ps1 persists LastStartDate/LastEndDate on close' {
    $appPsContent -match 'LastStartDate' -and $appPsContent -match 'LastEndDate'
}
...
$modulesToCheck = @('App.CoreAdapter.psm1','App.Auth.psm1','App.Config.psm1',
                    'App.Index.psm1','App.Export.psm1')
