...
ArchCheck 'ARCH-31' 'App.Config.psm1 supports env overrides (GENESYS_CORE_MODULE/CATALOG/SCHEMA)' {
    $appPs -match 'GENESYS_CORE_MODULE' -and
    $appPs -match 'GENESYS_CORE_CATALOG'
}

ArchCheck 'ARCH-32' 'All .psm1 modules use Set-StrictMode -Version Latest' {
    $modules = @($adapter, $auth, $config, $index, $export)
